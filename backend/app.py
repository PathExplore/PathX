...
class VolunteeringOpportunity(db.Model):
    ...
    signup_url = db.Column(db.String(255))
...
class SavedOpportunity(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), primary_key=True)
    opp_id = db.Column(db.Integer, db.ForeignKey('volunteering_opportunities.opp_id'), primary_key=True)
...
def get_opportunities():
    ...
    if category_filter:
        query = query.filter(
            db.or_(
                *[VolunteeringOpportunity.category.ilike(f"%{category}%") for category in category_filter]
            )
        )
    ...
    if skills_filter:
        query = query.filter(
            db.or_(
                *[VolunteeringOpportunity.skills_required.ilike(f"%{skill}%") for skill in skills_filter]
            )
        )
...
def get_dashboard():
    ...
    ).join(VolunteeringOpportunity, PastVolunteeringOpportunity.opp_id == VolunteeringOpportunity.opp_id) \
        .join(Organization, VolunteeringOpportunity.org_id == Organization.org_id) \
        .filter(PastVolunteeringOpportunity.user_id == user_id).all()

    saved_opps = db.session.query(
        SavedOpportunity,
        VolunteeringOpportunity.title,
        ...
        VolunteeringOpportunity.category,
        VolunteeringOpportunity.length,
        VolunteeringOpportunity.signup_url,  # Include signup_url in the query
        Organization.name.label('org_name')
    ).join(VolunteeringOpportunity, SavedOpportunity.opp_id == VolunteeringOpportunity.opp_id) \
        .join(Organization, VolunteeringOpportunity.org_id == Organization.org_id) \
    ...
    return jsonify({
        ...
        'savedOpportunities': [
        {
            ...
            'title': opp.title,
            'signup_url': opp.signup_url,  # Include signup_url in the response
            'date': opp.date,
            'location': opp.location,
            'category': opp.category,
            'organization': opp.org_name,
...
def save_opportunity():
    ...
    existing_saved = SavedOpportunity.query.filter_by(user_id=user_id, opp_id=opp_id).first()
    if existing_saved:
...
def get_accepted_events():
    ...
    events = db.session.query(
        ...
        .join(Organization, VolunteeringOpportunity.org_id == Organization.org_id) \
        .filter(AcceptedEvent.user_id == user_id).all()
    ...
    result = [
        {
            ...
            "date": event.VolunteeringOpportunity.date.strftime("%Y-%m-%d") if event.VolunteeringOpportunity.date else None,
...
def get_user_profile():
    ...
    total_hours = db.session.query(
        db.func.sum(VolunteeringOpportunity.length)
    ).join(PastVolunteeringOpportunity, PastVolunteeringOpportunity.opp_id == VolunteeringOpportunity.opp_id) \
        .filter(PastVolunteeringOpportunity.user_id == user_id).scalar()
...
def check_favorite():
    ...
    is_favorited = Favorite.query.filter_by(user_id=user_id, org_id=org_id).first() is not None
...
def remove_saved_opportunity():
    ...
    saved_opportunity = SavedOpportunity.query.filter_by(user_id=user_id, opp_id=opp_id).first()
...
def check_saved_opportunity():
    ...
    saved_opportunity = SavedOpportunity.query.filter_by(user_id=user_id, opp_id=opp_id).first() is not None
